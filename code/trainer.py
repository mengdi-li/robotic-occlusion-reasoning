...
if __name__ == "__main__":
    ...
    print("words: {} - ys: {} - nums: {} - occs: {}".format(words, ys, nums, occs))
    """
    ...
    while i < 1000:
        ...
        # select a object set
        # obj_set_idx = np.random.choice([0, 1], replace=False, p=[0.5, 0.5])
        # obj_set = cfg.TRAIN.OBJECT_SETS[obj_set_idx]

        all_objects = cfg.TRAIN.ALL_OBJECTS

        # select two classes
        classes = np.random.choice(
            [0, 1, 2], size=2, replace=False
        )  # uniform distribution over all entries
        classes = np.sort(classes)  # sort in ascending order
        assert classes[0] < classes[1]
        obj_class0 = all_objects[classes[0]]
        obj_class1 = all_objects[classes[1]]

        # get the object set for this episode
        obj_set = [np.random.choice(obj_class0), np.random.choice(obj_class1)]

        # make sure the obj_set is not a holdout pair
        while obj_set in cfg.TRAIN.HOLDOUT_PAIRS:
            # select two classes
            classes = np.random.choice(
                [0, 1, 2], size=2, replace=False
            )  # uniform distribution over all entries
        ...
        # select a query word
        query_word = np.random.choice(obj_set, p=[0.5, 0.5])

        # select a env mode
        mode_list = cfg.TRAIN.ENV_MODES
        env_mode = np.random.choice(mode_list, p=cfg.TRAIN.MODE_DIST)
        # env_mode = "two_occlusion"

        state = trainer.env.reset(
            pre_obj_set, big_obj=obj_set[0], small_obj=obj_set[1], mode=env_mode
        )

        pre_obj_set = obj_set
        # orientation = trainer.env.box.get_orientation()
        # pose = trainer.env.box.get_pose()
        pass
    toc_debug = time.time()
    print("toc_debug - tic_debug: {:.1f}s".format(toc_debug - tic_debug))
    """
